...
function PlayerHand (props: { player: Player, hand: Card[], hidden: boolean, onSelect: (card: Card) => void, activeCardId: string | null }) {
...
function CardSlotComp ({ cardSlot, onClick }: { cardSlot: CardSlot, onClick: (cardSlot: CardSlot) => void }) {
  const style = {
    ...cardSlotStyle,
...
function PlayerBoard (props: { player: Player, board: Board, onClickSlot: (cardSlot: CardSlot) => void }) {
  ...
  const rowSlots = rows.map(row => {
    ...
    return slots.map(cardSlot => <CardSlotComp key={cardSlot.index} onClick={props.onClickSlot} cardSlot={cardSlot} />)
  })
  const playerBoard = rowSlots.map((cardSlots, i) => <tr key={i}>{cardSlots}</tr>)
  return (
...
function Graveyard ({ graveyard }: { graveyard: Card[] }) {
  return (
    <div style={graveyard.length ? legacyPlayerHandCardStyle : cardSlotStyle}>
      <p>Graveyard</p>
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'center' }}>
        ...
        <div id="mid-col">
          ...
          <div style={{ height: 40 }}>
            {ctx.gameover && <h1 style={{ textAlign: 'center' }}>{ctx.gameover}</h1>}
          </div>
        ...
        <div id="left-col" style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
          ...
          <button onClick={() => moves.pass()} style={{ height: 60, borderRadius: 5, fontSize: 15 }}>Pass</button>
